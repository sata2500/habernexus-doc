...
export default function EditArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      const [articleResult, cats] = await Promise.all([
        getArticleToEdit(id),
        getCategories()
      ]);
      ...
      if (articleResult.success && articleResult.article) {
        ...
        setError(articleResult.error || "Makale yüklenemedi.");
        setLoading(false);
      }
    }
    load();
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-muted-foreground animate-pulse">Makale verileri yükleniyor...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        <Link href="/author/articles" className="h-10 w-10 flex items-center justify-center rounded-xl bg-muted hover:bg-muted/80 transition-colors">
            <ArrowLeft className="h-5 w-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold font-(family-name:--font-outfit)">Haberi Düzenle</h1>
          <p className="text-muted-foreground text-sm">Makalenizi güncelleyebilir veya durumunu değiştirebilirsiniz.</p>
        </div>
      </div>
